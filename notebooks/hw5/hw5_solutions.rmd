  ...
  for (l in lambdas) {
    for (pts in points) {
      ...
      p = ggplot(frame) +
        ...
        theme_bw() +
        labs(title=paste("Noisy sine curve, N =", n, ", noise = ", noise, ", bandwidth = ", l, ", points = ", pts))
      print(p)
    }
  }
}
...
noises = c(.1, .5, 1)                                                 # different amounts of noise to add to samples
lambdas = c(0.2, 0.5, 1, 3)                                           # manual bandwidths to try
lambda_selection_methods = c('nrd0', 'nrd', 'SJ')                     # bandwidth selection methods
points = c(as.integer(n / 10), as.integer(n / 4), as.integer(n / 2))  # different point counts to try
    ...
    for (l in lambdas) {
        for (pts in points) {
            ...
            p = ggplot(kde_result) +
              ...
              geom_line(data=data.frame(x=y, y=dnorm(y, mu1, sd1)), aes(x=x, y=y, color='left true')) +
              geom_line(data=data.frame(x=y, y=dnorm(y, mu2, sd2)), aes(x=x, y=y, color='right true')) +
              theme_bw() +
              labs(title=paste0("Bimodal ", w * 100, "% left ", (1 - w) * 100, "% right, N = ", n, ", noise = ", noise, ", bandwidth = ", l, ", points = ", pts))
            print(p)
        }
    }
  ...
  for (l in lambda_selection_methods) {
        for (pts in points) {
            ...
            p = ggplot(kde_result) +
              ...
              geom_line(data=data.frame(x=y, y=dnorm(y, mu2, sd2)), aes(x=x, y=y, color='right true')) +
              theme_bw() +
              labs(title=paste0("Bimodal, N = ", n, ", noise = ", noise, ", bandwidth = ", l, ", points = ", pts))
            print(p)
        }
    }
}
